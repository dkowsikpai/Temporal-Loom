...
parser.add_argument('--year', type=str, required=True, default="2010-2018")
    ...
    for line in reader:
        ...
        for item in line["answer"]:
            if start_year <= int(item["date"]) <= end_year:
                ...
                temp["answer"] = "In year " + item["date"] + ": " + item["answer"]
...
df.to_csv(f"./data/ft-{args.year}.csv", index=False)

df = pd.DataFrame(val_data)
df.to_csv(f"./data/ft-val-{args.year}.csv", index=False)
